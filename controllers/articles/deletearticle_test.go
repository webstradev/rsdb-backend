...
func TestDeleteArticle(t *testing.T) {
	tests := []struct {
		...
		{
			"DeleteArticle - sql error on DeleteArticle",
			"1",
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			// Register handler
			r.DELETE("/api/v1/articles/:articleId", DeleteArticle(env))

			// Create httptest request
			req, _ := http.NewRequest("DELETE", fmt.Sprintf("/api/v1/articles/%s", test.IdString), nil)
